...
### H5：congruence loss 优先增强低阶循环模式

在配对初始化实验中，congruence loss 会更早提高 $r=1$ 或邻近低阶目标模式能量。

### 后续协议工作假说

在训练能够完成拟合的范围内，提高 weight decay 可能改变有效残差清理速率和
$t_{\mathrm{dom}}$。该假说由 M4 的独立初始化 WD grid 与 M5 checkpoint 分支检验，不占用
固定 PDF 的 H1–H5 编号。
...
截至 2026-07-27，M2 已在 seed 1 的 0–50000 step 时间线上完成 H1–H4 所需的非 Fourier
函数测量。$t_{\mathrm{alg}}$ 的 100-step 网格估计为 100，$t_{\mathrm{dom}}$ 未达到，
且失稳 episode 显示函数量明显非单调。实际证据等级和限制见
[M2 失稳验证与函数空间分析](M2-disc.md)；Gate 2、Fourier、WD 分支和因果干预尚未运行。
